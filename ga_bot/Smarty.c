...
void readNParams(const Chromosome *chrom, int start, int N, uint8_t *out) {
    for (int i = 0; i < N; ++i) out[i] = paramFromChromosome(chrom, start + i * 8, chrom->length);
}

//chromosome structure:
// 15 rules are given priority value between 0 and 31, -> 4 bits * 15
#define Prio 60 // 4 * 15 = 60 bits used
// upon inference of the chromosomes, the rule functions are sorted with qsort
